...
all.datasets <- c("tirosh_melanoma_nonTumour")
...
all.datasets <- c("darmanis_glioblastoma_nonTumour", "JA_melanoma_nonTumour",
"darmanis_glioblastoma_allCells", "JA_melanoma_allCells", "tirosh_melanoma_allCells",
                  "li_crc_allCells")
...
for (dataset in all.datasets) {
  ...
  for (i in 3:length(alg)) {
    ...
    res_dataset_AIC <- cbind(res_dataset_AIC, res[2])
    res_dataset_BIC <- cbind(res_dataset_BIC, res[3])
  }
